...
def check_ip(
    ...
    try:
        ...
        check1 = requests.get(addr, headers=headers, proxies=proxies, timeout=10, verify=True).json()

        check2 = requests.get(addr, headers=headers, proxies=proxies, timeout=10, verify=True).json()
